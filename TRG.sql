...
AS
  BEGIN
    ...
    BEGIN
      ...
      VALUES (1, (SELECT inserted.noTournoi
                  FROM inserted))
    END
  END
GO
...
AS
  BEGIN
    ...
    BEGIN
      INSERT INTO dbo.tblTournoiCommandite
      VALUES ((SELECT inserted.noCommanditaire
               FROM inserted), 1)
    END
  END
GO
...
AS
  BEGIN
    ...
    BEGIN
      DECLARE @region INT
      EXECUTE psZoneGeographique SELECT inserted.codeNationalite
                                 FROM inserted, @region OUTPUT
      IF (@region = 1)
...
GO
DROP TRIGGER IF EXISTS dbo.SupprVueListeJoueurs
GO
CREATE TRIGGER dbo.SupprVueListeJoueurs
  ON dbo.VueListeJoueurs
  AFTER DELETE
AS
  BEGIN
    ...
    BEGIN
      ...
      IF (@region = 5)
        ...
        VALUES (SELECT *
                FROM inserted)
      IF (@region = 6)
        DELETE FROM dbo.tblJoueursOceanie
        WHERE dbo.tblJoueurs
    END
  END
